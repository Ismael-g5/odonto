 ...
 */
 ...
 if ( ! class_exists( 'Medicall_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Medicall_WebFont_Loader {
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'medicall_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'medicall_get_local_fonts_subfolder_name', 'fonts' );
			}
...
if ( ! function_exists( 'medicall_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function medicall_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Medicall_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'medicall_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function medicall_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new Medicall_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
