<?php
/**
 * Medicall Customizer Repeater Control.
 * 
 * @package Medicall
*/
...
if( ! class_exists( 'Medicall_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Medicall_Control_Repeater extends WP_Customize_Control {
    	public $type = 'medicall-repeater';
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'medicall' ), $this->row_label['value'] );
    		}
    	...
    	public function enqueue() {
            wp_enqueue_script( 'medicall-repeater', get_template_directory_uri() . '/inc/customizer/customizer-controls/repeater/assets/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'medicall-repeater', get_template_directory_uri() . '/inc/customizer/customizer-controls/repeater/assets/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'medicall' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="medicall-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'medicall' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php esc_attr_e( 'No Image Selected', 'medicall' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'medicall' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'medicall' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'medicall' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change Image', 'medicall' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add Image', 'medicall' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="medicall-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'medicall' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php esc_attr_e( 'No File Selected', 'medicall' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'medicall' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'medicall' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change File', 'medicall' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add File', 'medicall' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'medicall' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
